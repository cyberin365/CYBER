...
- 802.1D spanning tree protocol 
	...
	- then using those calculations, some ports are set in forwarding, others are blocked.
		- blocking (20s) - All ports start in the blocking state when a switch is powered on or a new link is connected. In this state, a port is sending BPDUs, listening for BPDUs, and processing BPDUs from neighboring switches. It does not forward any user data.p
...
## Installing Network
- Distribution frame
	- passive cable termination (punch down blocks, patch panels) to porevent signal reflection
